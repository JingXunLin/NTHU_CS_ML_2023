...
pip install -U "ray[tune, rllib] @ LINK_TO_WHEEL.whl"
"""
import argparse
...
if __name__ == "__main__":
    ...
    # Pygame can not pickle
    # ma_cartpole = make_multi_agent(TetrisSingleEnv(gridchoice="none", obs_type="grid", mode="human"))
    # register_env(
    #     "TetrisSingleEnv", lambda _: ma_cartpole({"num_agents": 2})
    # )
    register_env(
        "TetrisSingleEnv", lambda _: TetrisSingleEnv(gridchoice="none", obs_type="grid", mode="rgb_array")
    )
    
    single_dummy_env = TetrisSingleEnv(gridchoice="none", obs_type="grid", mode="rgb_array")
