...
def train(opt):
    ...
    now_model = DeepQNetwork()
    now_model.load_state_dict(torch.load('tetris'))
    prev_model = DeepQNetwork()
    prev_model.load_state_dict(now_model.state_dict())
    optimizer = torch.optim.Adam(now_model.parameters(), lr=opt.lr, betas=(0.9, 0.999))
